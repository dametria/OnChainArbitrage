...
export function validateConfig(): void {
  ...
  // Support both legacy shape (config.network.rpcUrl) and ChainConfig (rpcUrls.http)
  const rpcUrl = cfg.network?.rpcUrl || cfg.rpcUrls?.http || cfg.rpcUrl;
  if (!rpcUrl) {
    errors.push(
      `Missing RPC URL for network "${cfg.network?.name || cfg.name || "unknown"}"`
    );
  }
  ...
  // contracts may not exist when config is a pure ChainConfig
  if (
    cfg.contracts &&
    ...
    const envHint =
      process.env.NETWORK === "bsc"
        ...
        : process.env.NETWORK === "base"
          ...
          : "CONTRACT_ADDRESS";
    errors.push(
      `Missing FlashLoanArbitrage contract address for ${cfg.network?.name || cfg.name}. Set ${envHint} in .env`
    );
  }

  if (cfg.contracts && !cfg.contracts.aavePoolAddressProvider) {
    errors.push(
      `Missing aavePoolAddressProvider for ${cfg.network?.name || cfg.name}`
    );
  }

  // Check trading parameters when present
  if (cfg.trading) {
    if (cfg.trading.minProfitBps < 3) {
      errors.push(
        "minProfitBps too low - must be at least 3 bps (0.03%) for stablecoin strategy"
      );
    }
    if (cfg.trading.maxTradeSize < cfg.trading.minTradeSize) {
  ...
  const netName = cfg.network?.name || cfg.name || "unknown";
  const chainId = cfg.network?.chainId || cfg.chainId || "?";
  console.log(
    `✅ Configuration validated successfully (${netName} – chainId ${chainId})`
  );
  if (cfg.contracts?.flashLoanArbitrage) {
    console.log(`   Contract: ${cfg.contracts.flashLoanArbitrage}`);
  }
  if (cfg.contracts?.aavePoolAddressProvider) {
    console.log(
      `   Aave Provider: ${cfg.contracts.aavePoolAddressProvider || "(none)"}`
    );
  }
}
...
export function getTokenAddress(symbol: string): string {
  const cfg = config as any;
  const network = cfg.network?.name || cfg.name || "polygon";
  const tokens =
    network === "bsc"
      ? cfg.tokensBSC || cfg.tokens
      : network === "base"
        ? cfg.tokensBase || cfg.tokens
        : cfg.tokens || {};
  const address = tokens[symbol];
  if (!address) {
    throw new Error(
      `Token "${symbol}" not found in ${network} configuration`
    );
  }
  return address;
}
