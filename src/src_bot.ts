...
class ArbitrageBot {
  ...
  constructor() {
    // Initialize provider (prefer WSS from config.network.rpcUrl)
    const rpcUrl = (config && config.network && config.network.rpcUrl) || process.env.RPC_URL || "";

    if (!rpcUrl || typeof rpcUrl !== "string") {
      throw new Error(`rpcUrl must be provided as a string (config.network.rpcUrl or RPC_URL env)`);
    }
    ...
    if (rpcUrl.startsWith("ws://") || rpcUrl.startsWith("wss://")) {
      ...
      try {
        ...
        // Some provider implementations may not support .on in certain environments
        logger.debug("WebSocket provider event binding skipped:", e);
      }
    } else {
      // Fallback to HTTP
    ...
    try {
      ...
      for (const opportunity of opportunities) {
        ...
        logger.info("Opportunity Details:", {
          ...
          estimatedProfit: `${opportunity.profitUsd?.toFixed(2) ?? "0.00"}`,
          estimatedGas: `${opportunity.estimatedGasCost?.toFixed(2) ?? "0.00"}`,
          netProfit: `${opportunity.netProfit?.toFixed(2) ?? "0.00"}`,
        });
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.reason) {
          ...
          if (this.stats.failureReasons.hasOwnProperty(result.reason)) {
            // @ts-ignore
