...
export class TradeExecutor {
  ...
  private calculateFlashLoanAmount(
    ...
    if (!isStablecoin0 && !isStablecoin1) {
      // Neither side is a stable – use rough price of token0
  ...
  /**
   * Rough USD price of the network's native token (for gas cost conversion).
   * Network-agnostic via config.network.name when available, otherwise symbol map.
   */
  private estimateNativeTokenPriceUsd(): number {
    const name = (config.network?.name || "").toLowerCase();
    if (name === "polygon" || name === "matic") return 0.4;
    if (name === "bsc" || name === "bnb") return 600;
    if (name === "base" || name === "ethereum" || name === "arbitrum" || name === "optimism") return 2000;
    if (name === "avalanche" || name === "avax") return 30;
    if (name === "fantom" || name === "ftm") return 0.5;
    // Fallback – most L2s use ETH
    return 2000;
  }
  ...
  private async validateProfitability(
    ...
    try {
      ...
      // Estimated gas cost in USD (use native token price – network-agnostic)
      ...
      const nativePrice = this.estimateNativeTokenPriceUsd();
      const gasCostUsd = gasCostNative * nativePrice;
      
      // Calculate trade size in USD using token0 price
      const flashLoanAmountTokens = parseFloat(ethers.formatEther(flashLoanAmount));
      const token0Price = this.estimateToken0PriceUsd(opportunity);
      const tradeSizeUsd = flashLoanAmountTokens * token0Price;
  ...
  async executeTrade(
    ...
    try {
      ...
      // 💰 LOG TRADE SIZE DETAILS (network-agnostic)
      const flashLoanAmountFormatted = ethers.formatEther(flashLoanAmount);
      const token0Price = this.estimateToken0PriceUsd(opportunity);
      const tradeSizeUSD = parseFloat(flashLoanAmountFormatted) * token0Price;
      ...
      try {
        ...
        const netProfitTokens = parseFloat(ethers.formatEther(simulationResult.netProfit));
        const nativePrice = this.estimateNativeTokenPriceUsd();
        const netProfitUSD = netProfitTokens * nativePrice;
        ...
        logger.success(`   Net profit: ${netProfitTokens.toFixed(6)} tokens ($${netProfitUSD.toFixed(2)})`);
